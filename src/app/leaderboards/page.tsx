...
import { LEADERBOARD_DATA, GAME_LEADERBOARDS, PLATFORM_GAMES } from "@/data/platform";
...
export default function LeaderboardsPage() {
  ...
  const board = useMemo(() => {
    if (gameFilter === "overall") return LEADERBOARD_DATA[mode];
    return GAME_LEADERBOARDS[gameFilter]?.[mode] ?? [];
  }, [mode, gameFilter]);
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        <div className="text-center p-1.5 bg-slate-950 border-2 border-black rounded-lg shadow-[1px_1px_0px_#000]">
                          <p className="text-[6px] font-black text-slate-550 uppercase tracking-widest mb-0.5">XP</p>
                          <p className="text-[10px] font-space font-black text-brand-orange">{player.xp.toLocaleString()}</p>
                        </div>
                        <div className="text-center p-1.5 bg-slate-950 border-2 border-black rounded-lg shadow-[1px_1px_0px_#000]">
                          <p className="text-[6px] font-black text-slate-550 uppercase tracking-widest mb-0.5">Wins</p>
                          <p className="text-[10px] font-space font-black text-slate-55">{player.wins}</p>
                        </div>
                        <div className="text-center p-1.5 bg-slate-950 border-2 border-black rounded-lg shadow-[1px_1px_0px_#000]">
                          <p className="text-[6px] font-black text-slate-550 uppercase tracking-widest mb-0.5">Streak</p>
                    ...
                    {rest.map(player => (
                      <div key={player.rank} className="flex items-center justify-between p-3 hover:bg-slate-900/40 transition-colors">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="h-7 w-7 rounded-full bg-slate-950 border-2 border-black flex items-center justify-center text-slate-450 font-black text-[10px] shrink-0 shadow-[1px_1px_0px_#000000]">{player.name.charAt(0)}</div>
                          <div className="min-w-0">
                          ...
                          <tr key={player.rank} className="group hover:bg-slate-900/10 transition-colors">
                            ...
                            <td className="px-5 py-3">
                              <div className="flex items-center gap-2.5">
                                <div className="h-7 w-7 rounded-full bg-slate-950 border-2 border-black flex items-center justify-center text-slate-500 font-black text-[10px] shadow-[1px_1px_0px_#000]">{player.name.charAt(0)}</div>
                                <div>
                                  ...
                                  <p className="text-[7px] font-black text-slate-550 uppercase tracking-widest flex items-center gap-1"><Star className="w-2 h-2" /> Tier Elite</p>
                                </div>
                              </div>
                            </td>
